...
export default function ArticleCart({ ...art }) {
    ...
    return (
        <>
            <NavLink to={`/articles/${art.shortName}`} className='group flex gap-x-2.5 sm:block p-2.5 md:pb-2 bg-white dark:bg-zinc-700 shadow-normal rounded-2xl'>
                <div className='relative shrink-0 w-32 h-32 sm:w-auto sm:h-auto sm:mb-4 rounded-2xl rounded-bl-4xl overflow-hidden'>
                    {
                        (art.cover) ? (
                            <img loading='lazy' className="h-full sm:h-auto object-cover" src={`${DataUrl}/courses/covers/${art.cover}`} alt="" />
                        ) : (
                            <img loading='lazy' className="h-full sm:h-auto object-cover" src="/images/blogs/nocover.png" alt="" />
                        )
                    }
                    <div className='absolute inset-0 w-full h-full md:flex-center hidden invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all delay-75 bg-gradient-to-r from-orange-200/80 to-orange-300/80'>
                        <svg className='w-[138px] h-[54px] text-amber-900' viewBox="0 0 138 55" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className='flex flex-col sm:flex-row items-start justify-between w-full'>
                    <h2 className='font-DanaMedium md:font-Dana ml-1.5 sm:ml-0 line-clamp-2 md:max-w-[193px] text-sm/7 md:text-base text-zinc-700 dark:text-white mt-2.5 sm:mt-0'>{art.title}</h2>
                    <div className='hidden sm:flex gap-5'>
                        <span className='hidden lg:block w-px h-[61px] bg-gray-100 dark:bg-white/10'></span>
                        <div className='flex flex-col ml-3 lg:ml-[18px] text-teal-600 dark:text-emerald-500 text-sm text-left '>
                            <span className='font-DanaBold md:text-xl lg:text-2xl'>
                    ...
                    <div className='flex items-end justify-between w-full sm:hidden border-t border-gray-100 dark:border-t-white/10 pt-[18px] pb-1.5'>
                        <span className='text-teal-600 dark:text-emerald-500 text-xs flex gap-x-0.5'>
                            <span className=''>
                                {moment(art.createdAt).locale('fa').format('jD')}
                            </span>
                            <span className=''>
                        ...
                        <NavLink to={art.shortName} className='flex items-center font-DanaMedium text-xs h-5 rounded-md ml-1.5 pr-2.5 pl-2 gap-x-1 bg-orange-200/20 text-orange-300'>
                            <span>
                                مطالعه
                            </span>
            ...
            {/* <div className="bg-white border border-gray-200 rounded-lg shadow dark:bg-zinc-700 dark:border-gray-700 flex md:flex-col items-center p-2">
                <div className="w-1/3 md:w-full">
                    <NavLink to={art.shortName}>
                        {
                            (art.cover) ? (
                                <img loading='lazy' className="rounded-2xl rounded-bl-4xl h-[129px] md:h-full object-cover" src={`${DataUrl}/courses/covers/${art.cover}`} alt="" />
                            ) : (
                                <img loading='lazy' className="rounded-2xl rounded-bl-4xl h-[129px] md:h-full object-cover" src="/images/blogs/nocover.png" alt="" />
                            )
                        }
                    </NavLink>
                </div>
